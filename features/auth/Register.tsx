...
const MUNICIPIOS = [
  'Uruapan', 'Zamora', 'Apatzingán', 'Los Reyes', 'La Piedad',
  'Lázaro Cárdenas', 'Morelia', 'Zitácuaro', 'Pátzcuaro',
  'Jacona', 'Jiquilpan', 'Sahuayo', 'Puruarán', 'Otros',
];

const Register: React.FC = () => {
  const navigate = useNavigate();
  const { sendOtp, verifyRegisterOtp } = useAuth();
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    if (phone.replace(/\D/g, '').length < 10) { setError('Ingresa un número de teléfono válido de 10 dígitos.'); return; }
    if (role === 'producer' && !location) { setError('Por favor selecciona tu municipio.'); return; }
  ...
  // ── Paso 2: Verificar OTP + Crear perfil ───────────────────────────────
  const handleVerifyOtp = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (otp.replace(/\D/g, '').length !== 6) { setError('El código debe tener 6 dígitos.'); return; }

    setIsLoading(true);
    try {
      await verifyRegisterOtp(formattedPhone, otp.trim(), hash, role, name.trim(), location);
      ...
      <div className="w-full max-w-[580px] bg-white rounded-2xl shadow-xl shadow-gray-200/50 overflow-hidden flex flex-col ring-1 ring-black/5">
        ...
        <div className="p-6 sm:p-8 lg:p-10 flex flex-col gap-5">
          ...
          <div className="flex items-center gap-3">
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-colors ${step === 'form' ? 'bg-primary text-white' : 'bg-green-100 text-green-700'}`}>
          ...
          {step === 'form' && (
            <form onSubmit={handleSendOtp} className="flex flex-col gap-4">
              <div>
                <h1 className="text-2xl font-extrabold tracking-tight mb-1">Crear cuenta</h1>
                <p className="text-sm text-gray-500">Verificaremos tu número de teléfono con un código SMS.</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['producer', 'buyer'] as const).map((r) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setRole(r)}
                    className={`p-3 rounded-xl border-2 text-left transition-all flex flex-col gap-1 ${role === r ? 'border-primary bg-primary/5' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <span className="material-symbols-outlined text-xl text-primary">{r === 'producer' ? 'agriculture' : 'storefront'}</span>
                    <span className="text-sm font-bold">{r === 'producer' ? 'Productor' : 'Comprador'}</span>
                    <span className="text-xs text-gray-500">{r === 'producer' ? 'Vendo mis cosechas' : 'Busco productos'}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-sm"
                  required
                />
              </div>
              ...
              {/* Teléfono */}
              <div className="flex flex-col gap-1.5">
                <label htmlFor="reg-phone" className="text-sm font-semibold text-gray-700">Número de teléfono</label>
                <div className="flex items-center gap-0 border border-gray-300 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-primary focus-within:border-primary transition-all">
                  <span className="px-4 py-3 bg-gray-50 text-sm text-gray-600 border-r border-gray-300 font-medium select-none">+52</span>
                  <input
                    id="reg-phone" type="tel" inputMode="numeric"
                    value={phone} onChange={(e) => setPhone(e.target.value)}
                    placeholder="10 dígitos" maxLength={14}
                    className="flex-1 px-4 py-3 text-sm outline-none bg-white"
                    required
                  />
                </div>
                <p className="text-xs text-gray-400">Un mismo número = una sola cuenta. No puedes crear duplicados.</p>
              </div>

              {/* Municipio (solo productor) */}
              {role === 'producer' && (
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="location" className="text-sm font-semibold text-gray-700">Municipio</label>
                  <select
                    id="location" value={location}
                    onChange={(e) => setLocation(e.target.value)}
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-sm bg-white"
                    required
                  >
              ...
              >
                {isLoading ? (
                  ...
                  <><span className="material-symbols-outlined text-base">sms</span> Continuar con SMS</>
                )}
              </button>
          ...
          {step === 'otp' && (
            <form onSubmit={handleVerifyOtp} className="flex flex-col gap-5">
              <div>
                <h1 className="text-2xl font-extrabold tracking-tight mb-1">Confirma tu número</h1>
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="reg-otp" className="text-sm font-semibold text-gray-700">Código de verificación</label>
                <input
                  id="reg-otp" type="text" inputMode="numeric"
                  value={otp}
              ...
              >
                {isLoading ? (
                  ...
                  <><span className="material-symbols-outlined text-base">verified_user</span> Verificar y crear cuenta</>
                )}
              </button>
